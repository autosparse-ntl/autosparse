...
CSV_HEADER = [
    ...
    "attack","rank","dora_alpha","epoch","epochs","lr","batch_size",
    ...
    "num_dora_layers","targeted_layers","missing_keys","unexpected_keys"
]
...
class DoRALinear(nn.Module):
    def __init__(self, linear, rank, alpha):
        ...
        with torch.no_grad():
            mag = linear.weight.norm(dim=1)

        self.magnitude = nn.Parameter(mag.clone())

    def forward(self, x):
        delta = self.B @ self.A
        adapted = self.linear.weight + self.scale * delta
        direction = adapted / (adapted.norm(dim=1, keepdim=True) + 1e-6)
...
class DoRAConv2d(nn.Module):
    def __init__(self, conv, rank, alpha):
        ...
        flat_dim = conv.in_channels * kh * kw

        self.A = nn.Parameter(torch.randn(rank, flat_dim) * 0.01)
        self.B = nn.Parameter(torch.zeros(out_ch, rank))

        with torch.no_grad():
            mag = conv.weight.view(out_ch, -1).norm(dim=1)

        self.magnitude = nn.Parameter(mag.clone())

    def forward(self, x):
        out_ch = self.conv.out_channels
        base = self.conv.weight.view(out_ch, -1)
        delta = self.B @ self.A

        adapted = base + self.scale * delta
        direction = adapted / (adapted.norm(dim=1, keepdim=True) + 1e-6)

        weight_flat = self.magnitude.unsqueeze(1) * direction
        weight = weight_flat.view_as(self.conv.weight)
...
def replace_last_linear(model, rank, alpha):
    ...
    if not names:
        raise RuntimeError("No Linear layer found for simple_dora")
    ...
    setattr(parent, parts[-1], DoRALinear(old, rank, alpha))

    return 1, [target]

def replace_all_dora(module, rank, alpha, prefix=""):
    count = 0
    names = []
    ...
    for name, child in list(module.named_children()):
        ...
        if isinstance(child, nn.Conv2d):
            setattr(module, name, DoRAConv2d(child, rank, alpha))
            count += 1
            names.append(full)
        elif isinstance(child, nn.Linear):
            setattr(module, name, DoRALinear(child, rank, alpha))
            count += 1
            names.append(full)
        else:
            c, n = replace_all_dora(child, rank, alpha, full)
            count += c
            names.extend(n)
...
def build_model(info, rank, attack):
    ...
    if attack == "simple_dora":
        n_dora, layers = replace_last_linear(model, rank, alpha)
    elif attack == "strong_dora":
        n_dora, layers = replace_all_dora(model, rank, alpha)
    else:
        raise ValueError(attack)

    return model, ckpt_path, missing, unexpected, n_dora, layers, alpha
...
def train_one(model, train_loader, test_loader, csv_path, logger, info, attack,
              rank, alpha, ckpt_path, missing, unexpected, n_dora, layers):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, y in tqdm(
            train_loader,
            desc=f"{info['name']} {attack} R{rank} E{epoch}",
            leave=False,
        ):
            x = x.to(DEVICE, non_blocking=True)
            y = y.to(DEVICE, non_blocking=True)
...
def output_files(attack):
    if attack == "simple_dora":
        return (
            os.path.join(OUT_DIR, "master_simple_dora_epoch_results.csv"),
            os.path.join(OUT_DIR, "simple_dora.log"),
        )

    if attack == "strong_dora":
        return (
            os.path.join(OUT_DIR, "master_strong_dora_epoch_results.csv"),
            os.path.join(OUT_DIR, "strong_dora.log"),
        )
...
def main():
    ...
    parser.add_argument("--attack", default="both", choices=["simple_dora", "strong_dora", "both"])
    ...
    attacks = ["simple_dora", "strong_dora"] if args.attack == "both" else [args.attack]
    loader_cache = {}
    ...
    for attack in attacks:
        ...
        for info in CHECKPOINTS:
            ...
            for rank in RANKS:
                ...
                model, ckpt_path, missing, unexpected, n_dora, layers, alpha = build_model(
                    info, rank, attack
                )
